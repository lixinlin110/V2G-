...
def build_seasonal_data():
    ...
    seasonal_data = {}
    ev_cfg = cfg.get('ev', {})
    v2g_rate = ev_cfg.get('v2g_participation_rate', 0.3)
    # 保守口径：并非所有已参与车辆都能在每个时段按净负荷完全响应
    dispatch_factor = ev_cfg.get('v2g_dispatch_factor', 0.6)
    ...
    for season, base_loads in seasonal_base_loads.items():
        ...
        with_v2g = []
        for i in range(24):
            # 仅将参与V2G的车辆按净负荷结算，其他车辆保持常规充电负荷
            non_participating_load = ev_load[i] * (1 - v2g_rate)
            participating_load = (
                ev_load[i] * v2g_rate * (1 - dispatch_factor) +
                net_ev_load[i] * v2g_rate * dispatch_factor
            )
            effective_ev_load = max(non_participating_load + participating_load, 0)
            with_v2g.append(int(base_loads[i] + effective_ev_load))
        v2g_profit = round(sum(item['profit'] for item in schedule if item['action'] == 2), 2)
        charging_cost = round(sum(abs(item['profit']) for item in schedule if item['action'] == 0), 2)
